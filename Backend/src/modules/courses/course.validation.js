const { z } = require("zod");
const { courseCategories, courseLevels, courseLanguages, courseSortFields, courseStatus } = require("../../constants/course.constants");
...
const updateCourseSchema = createCourseSchema.partial().refine((data) => Object.keys(data).length > 0, {
...
const getMyCoursesSchema = z.object({
    ...
    status: z.enum(courseStatus, { message: 'Invalid course status' }).optional(),
    category: z.enum(courseCategories, { message: 'Course category is not valid' }).optional(),
    level: z.enum(courseLevels, { message: 'Course level is not valid' }).optional(),
});
...
const restoreArchivedCourseSchema = z.object({
    courseId: z.string().regex(/^[0-9a-fA-F]{24}$/, {
        message: 'Invalid course ID',
    }),
});
